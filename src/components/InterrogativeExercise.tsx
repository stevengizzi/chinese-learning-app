...
interface InterrogativeExerciseProps {
  ...
  onSubmitAttempt: (attempt: InterrogativeAttempt) => void;
  onEnd: () => void;
}
...
export function InterrogativeExercise({
  item,
  session,
  onSubmitAttempt,
  onEnd,
}: InterrogativeExerciseProps) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    const result = gradeInterrogativeAnswer(
      userAnswer,
      item.chineseAnswer,
      item.questionType,
      item.alternateAnswers
    );
    ...
    const attempt: InterrogativeAttempt = {
      itemId: item.id,
  ...
  const questionColors = QUESTION_COLORS[item.questionType];
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
          ...
          {showHints && (
            <div className="flex justify-center mb-4">
              <span
                className={`px-4 py-1.5 rounded-full text-sm font-medium border-2 ${questionColors.bg} ${questionColors.text} ${questionColors.border}`}
              >
                {QUESTION_SHORT_LABELS[item.questionType]}
              </span>
            </div>
          )}
          ...
          <div className="text-center mb-8">
            ...
            {!showHints && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                {QUESTION_DISPLAY_NAMES[item.questionType]}
              </p>
            )}
          </div>
                ...
                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                  ...
                  {feedbackState.missingMarkers.length > 0 && (
                    <p className="text-sm text-center text-red-700 dark:text-red-300">
                      Missing question word: <span className="font-bold">{feedbackState.missingMarkers.join(', ')}</span>
                    </p>
                  )}
                  {feedbackState.wrongMarkers.length > 0 && (
                    <p className="text-sm text-center text-red-700 dark:text-red-300">
                      Wrong question word: <span className="font-bold">{feedbackState.wrongMarkers.join(', ')}</span>
                    </p>
                  )}
                </div>
              )}
