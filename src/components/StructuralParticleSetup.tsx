...
import type { StructuralParticleConfig, StructuralParticleItem, StructuralParticle } from '../types/structuralParticle';
import {
  ALL_STRUCTURAL_PARTICLES,
  PARTICLE_INFO,
  DEFAULT_STRUCTURAL_PARTICLE_CONFIG,
} from '../types/structuralParticle';
import { filterByParticles, getCountsByParticle } from '../lib/structuralParticleParser';

interface StructuralParticleSetupProps {
  items: StructuralParticleItem[];
  onStart: (config: StructuralParticleConfig) => void;
  onBack: () => void;
}

export function StructuralParticleSetup({ items, onStart, onBack }: StructuralParticleSetupProps) {
  const [config, setConfig] = useState<StructuralParticleConfig>(DEFAULT_STRUCTURAL_PARTICLE_CONFIG);

  const counts = getCountsByParticle(items);
  const filteredCount = filterByParticles(items, config.selectedParticles).length;

  const toggleParticle = (particle: StructuralParticle) => {
    setConfig(prev => {
  ...
  const selectAll = () => {
    setConfig(prev => ({ ...prev, selectedParticles: [...ALL_STRUCTURAL_PARTICLES] }));
  };
  ...
  const renderParticleButton = (particle: StructuralParticle) => {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-300">
              Master the three "de" particles: 的, 地, 得
            </p>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {ALL_STRUCTURAL_PARTICLES.map(renderParticleButton)}
            </div>
          </div>
