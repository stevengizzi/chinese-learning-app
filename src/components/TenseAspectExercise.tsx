...
import { ASPECT_DISPLAY_NAMES, ASPECT_COLORS, ASPECT_SHORT_LABELS } from '../types/tenseAspect';
import { gradeTenseAspectAnswer } from '../lib/tenseAspectGrader';
...
interface TenseAspectExerciseProps {
  item: TenseAspectItem;
  session: TenseAspectSession;
  onSubmitAttempt: (attempt: TenseAspectAttempt) => void;
  onEnd: () => void;
}
...
export function TenseAspectExercise({
  item,
  session,
  onSubmitAttempt,
  onEnd,
}: TenseAspectExerciseProps) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    setFeedbackState({
      ...
      missingMarkers: result.markerAnalysis.missingMarkers,
      wrongMarkers: result.markerAnalysis.wrongMarkers,
    });

    const attempt: TenseAspectAttempt = {
      itemId: item.id,
      aspectType: item.aspectType,
      userAnswer: userAnswer.trim(),
      correctAnswer: item.chineseAnswer,
      isCorrect: result.isCorrect,
      markerCorrect: result.markerAnalysis.markerCorrect,
      timeMs,
    };
  ...
  const handleNext = useCallback(() => {
    ...
    const timeMs = Date.now() - startTime;
    const result = gradeTenseAspectAnswer(feedbackState.userAnswer, item.chineseAnswer, item.aspectType);
  ...
  const aspectColors = ASPECT_COLORS[item.aspectType];
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
          ...
          {showHints && (
            <div className="flex justify-center mb-4">
              <span
                className={`px-4 py-1.5 rounded-full text-sm font-medium border-2 ${aspectColors.bg} ${aspectColors.text} ${aspectColors.border}`}
              >
                {ASPECT_SHORT_LABELS[item.aspectType]}
              </span>
            </div>
          )}
          ...
          <div className="text-center mb-8">
            ...
            {!showHints && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                {ASPECT_DISPLAY_NAMES[item.aspectType]}
              </p>
            )}
          </div>
            ...
            <input
              ...
              className={`w-full p-4 text-2xl text-center border-2 rounded-xl transition-all ${
                feedbackState.shown
                  ...
                  : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-800'
          ...
          {feedbackState.shown && (
            ...
            >
              ...
              {/* Marker analysis for incorrect answers */}
              {!feedbackState.isCorrect && (
                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                  ...
                  {feedbackState.missingMarkers.length > 0 && (
                    <p className="text-sm text-center text-red-700 dark:text-red-300">
                      Missing marker: <span className="font-bold">{feedbackState.missingMarkers.join(', ')}</span>
                    </p>
                  )}
                  {feedbackState.wrongMarkers.length > 0 && (
                    <p className="text-sm text-center text-red-700 dark:text-red-300">
                      Wrong marker: <span className="font-bold">{feedbackState.wrongMarkers.join(', ')}</span>
                    </p>
                  )}
                </div>
              )}
