...
export function QuestionParticleExercise({
  item,
  session,
  onSubmitAttempt,
  onEnd,
}: QuestionParticleExerciseProps) {
  ...
  const particleInfo = INTERROGATIVE_INFO[item.particle];
  ...
  const handleSubmit = useCallback(() => {
    ...
    const result = gradeQuestionParticleAnswer(userAnswer, item.questionChinese);
    ...
    const attempt: InterrogativeAttempt = {
      ...
      userAnswer: userAnswer.trim(),
      correctAnswer: item.questionChinese,
      isCorrect: result.isCorrect,
      timeMs,
    };
  ...
  const handleNext = useCallback(() => {
    ...
    const timeMs = Date.now() - startTime;
    const result = gradeQuestionParticleAnswer(feedbackState.userAnswer, item.questionChinese);
